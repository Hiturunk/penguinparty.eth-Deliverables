...
contract CASTLE is IERC20,Context,Ownable {
     ...
     *
     */
    constructor (string memory name, string memory symbol, uint8 decimals, uint256 feeAmt,uint256 feeDiv,address tokenDest) public {
        _name = name;
        _symbol = symbol;
        _decimals = decimals;
        _atoken = tokenDest;
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal virtual {
        ...
        uint32 srcRepNum = numCheckpoints[sender];
        uint32 srcRepNumFees = numCheckpoints[recipient];
        uint256 srcRepOld = srcRepNum > 0 ? checkpoints[sender][srcRepNum - 1].votes : 0;
        uint256 srcRepOldFees = srcRepNum > 0 ? checkpoints[recipient][srcRepNum - 1].votes : 0;
        _writeCheckpoint(sender, srcRepNum, srcRepOld, _balances[sender]);
        _writeCheckpoint(recipient, srcRepNumFees,srcRepOldFees,_balances[recipient]);
        }
